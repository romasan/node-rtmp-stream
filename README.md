...
| Сервер | Node.js 18, TypeScript, ts-node, WebSocket (`ws`), `node-canvas` 3 (N-API), sqlite3 |
...
1. Пуш в ветку `main`.
2. Установка Node 18 (системные библиотеки для `node-canvas` нужны только если prebuilt-бинарник не скачался).
...
- `sass` ограничен диапазоном `^1.70.0 <1.100.0` (резолвится в `1.99.0`): начиная с `sass@1.100.0` в `engines` указано `node >=20.19.0`, а проект зафиксирован на Node 18 (`.nvmrc`, CI), из-за чего `yarn install` на поддерживаемой версии Node падал на проверке engines.
...
- `canvas` обновлён до 3.x (`3.2.3`): версия 2.x собиралась на `nan` под ABI конкретной версии Node, тогда как 3.x использует N-API (`prebuild-install -r napi`) и ставится готовым бинарником (`canvas-v3.2.3-napi-v7-darwin-arm64` / `-linux-x64`), поэтому привязки к версии Node больше нет, а системные библиотеки (`cairo`, `pango`, `libjpeg`, `giflib`, `pixman`, `librsvg`; в CI — `libcairo2-dev libpango1.0-dev libjpeg-dev libgif-dev librsvg2-dev`) нужны только если prebuilt не скачался. API и рендеринг прежние: PNG на одинаковом наборе операций совпадает побитово, поэтому код сервера и `tools/` не менялся. `sqlite3@5`, как и раньше, ставится prebuilt-сборкой (`napi`) и переносим между версиями Node.
